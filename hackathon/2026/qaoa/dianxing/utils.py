...
def warm_theta_from_bitsx(bits01: np.ndarray, warm_c: float) -> np.ndarray:
    c = float(np.clip(warm_c, 0.0, 1.0))
    b = np.asarray(bits01, dtype=np.float64)
    z = 2 * b - 1
    x = 1.0 / (1.0 + np.exp(-4 * c * z))
    x = np.clip(x, 1e-6, 1.0 - 1e-6)
    return 2.0 * np.arcsin(np.sqrt(x))



def warm_theta_from_bitsy(bits01: np.ndarray, warm_c: float, problem=None, eps=1e-6):
    ...
    if problem is not None:
        ...
        degree = degree / (degree.max() + eps)  # 归一化

        # 度数越高 → cᵢ 越大
        c_array = base_c * (0.9 + 0.1 * degree)
    else:
        c_array = np.full(n, base_c)

    c_array = np.clip(c_array, 0.1, 0.95)
    x = (1 - c_array) * 0.5 + c_array * bits01
...
def warm_theta_from_bits2(
    bits01: np.ndarray,
    warm_c: float,
    problem=None,  # 传入 problem 就能算逐比特 cᵢ
    eps: float = 1e-6
) -> np.ndarray:
    # 基础全局 c
    base_c = float(np.clip(warm_c, 0.0, 1.0))
    ...
    # ===================== 逐比特自适应 cᵢ =====================
    if problem is not None and hasattr(problem, 'h'):
        # 用【每个比特的目标敏感度】自动计算 c_i
        h = problem.h  # shape (k, n)
        h_amplitude = np.mean(np.abs(h), axis=0)  # 每个比特的平均场强
        h_norm = h_amplitude / (h_amplitude.max() + eps)  # 归一化 0~1

        # 重要比特 → c 大；不重要 → c 小
        c_array = base_c * (0.7 + 0.3 * h_norm)  # 范围：0.4*base ~ 1.0*base
    else:
...
def warm_theta_from_bits2x(
    bits01: np.ndarray,
    warm_c: float,
    problem=None,  # 必须包含 h, J, edges
    eps: float = 1e-6
) -> np.ndarray:
    # 基础全局热启动系数
    base_c = float(np.clip(warm_c, 0.0, 1.0))
    ...
    # ===================== 【升级】融合 h + J 综合重要性 =====================
    if problem is not None and hasattr(problem, 'h') and hasattr(problem, 'J'):
        h = problem.h  # shape: (k, n)
        J = problem.J  # shape: (k, E)
        edges = problem.edges  # shape: (E, 2)

        # 1. 计算【节点场幅值】每个比特的平均场强
        h_amplitude = np.mean(np.abs(h), axis=0)  # (n,)

        # 2. 计算【耦合场幅值】每个节点相连边的平均耦合强度（新增）
        J_amplitude = np.zeros(n, dtype=np.float64)
        edge_count = np.zeros(n, dtype=np.int32)
        # 遍历所有边，累加每个节点的耦合强度
        for e_idx, (u, v) in enumerate(edges):
            j_val = np.mean(np.abs(J[:, e_idx]))  # 该边在k个目标的平均强度
            J_amplitude[u] += j_val
            J_amplitude[v] += j_val
            edge_count[u] += 1
            edge_count[v] += 1
        # 平均化，避免孤立节点除0
        J_amplitude = np.divide(J_amplitude, edge_count, 
                                out=np.zeros_like(J_amplitude), 
                                where=edge_count > 0)

        # 3. 综合重要性 = 场强 + 耦合强度 平均融合
        total_amplitude = (h_amplitude + J_amplitude)/2

        # 4. 归一化到 [0,1]
        total_norm = total_amplitude / (np.max(total_amplitude) + eps)

        # 5. 逐比特自适应 c_i
        c_array = base_c * (0.7 + 0.3 * total_norm)
    else:
        # 无问题信息 → 全局统一c
        c_array = np.full(n, base_c, dtype=np.float64)
...
def build_qaoa_circuit_from_projected_ising2(
    ...
    *,
    ...
    if warm_bits01 is None:
        ...
        for q, th in enumerate(thetas):
            circ += RY(float(th+np.pi/15)).on(q)
...
def warm_theta_from_bits_smooth(
    bits01: np.ndarray,
    warm_c: float,
    problem=None,
    eps: float = 1e-6
    ...
    if problem is not None and hasattr(problem, 'h') and hasattr(problem, 'J'):
        ...
        # 节点场重要性
        h_amp = np.mean(np.abs(h), axis=0)

        # 边耦合重要性
        J_amp = np.zeros(n)
        deg = np.zeros(n, dtype=int)
        for e, (u, v) in enumerate(edges):
            j_val = np.mean(np.abs(J[:, e]))
            J_amp[u] += j_val
            J_amp[v] += j_val
            deg[u] += 1
            deg[v] += 1
        J_amp = np.divide(J_amp, deg, out=np.zeros_like(J_amp), where=deg > 0)

        # 综合重要性
        s = (h_amp + J_amp) / 2
        s_norm = s / (np.max(s) + eps)
        c_array = base_c * (0.4 + 0.6 * s_norm)
    else:
        c_array = np.full(n, base_c)
    ...
    # ===================== 平滑非线性公式（不早熟！） =====================
    x = 0.5 + c_array * (bits01 - 0.5) * np.cos(np.pi * 0.5 * (1 - c_array))
    x = np.clip(x, 1e-6, 1 - 1e-6)

    return 2.0 * np.arcsin(np.sqrt(x))

def build_qaoa_circuit_from_projected_ising22(
    problem: IsingMOOProblem,
    j_raw: np.ndarray,
    h_raw: np.ndarray,
    *,
    ...
    warm_bits01: np.ndarray | None = None,
    warm_c: float = 0.5,
    current_round: int = 0,      # 新增：当前迭代轮次
    total_rounds: int = 3,      # 新增：总轮次数
) -> Circuit:
    """Build QAOA circuit from already projected scalarized Ising coefficients."""
    n = int(problem.n)
    m = int(problem.m)
    ...
    # -------------------------------------------------------------------------
    # 改进 1：轮次退火 warm_c（前期小，后期大）
    # -------------------------------------------------------------------------
    progress = current_round / max(total_rounds, 1)
    warm_c = 0.4 + 0.5 * progress  # 从 0.4 → 0.9 平滑上升
    warm_c = np.clip(warm_c, 0.3, 0.9)
    ...
    if warm_bits01 is None:
        ...
        bits01 = np.asarray(warm_bits01, dtype=np.int8).reshape(n)
        # 改进 3：使用【平滑非线性热启动】，不早熟
        thetas = warm_theta_from_bits2(bits01, warm_c, problem=problem)
        for q, th in enumerate(thetas):
            circ += RY(float(th)).on(q)
    ...
    for layer in range(p):
        ...
        # ---------------------------------------------------------------------
        # 改进 2：层间余弦退火（每层 β, γ 自动调整）
        # ---------------------------------------------------------------------
        beta = beta * np.cos(np.pi * layer / (2 * max(p, 1)))
        gamma = gamma * (0.2 + 0.8 * (layer + 1) / p)

        # 代价层缩放（保留）
        gamma_eff = -scale_gamma(gamma, edges=problem.edges, n=n, J=j, h=h)
